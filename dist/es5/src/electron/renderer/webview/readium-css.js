...
var isTwoPageSpread = function () {
    ...
    var scrollElement = (0, exports.getScrollingElement)(win.document);
    var bodyComputedStyle = win.getComputedStyle(win.document.body);
    var bodyWidth = parseInt(bodyComputedStyle.width, 10);
    var paginatedTwo = docColumnCount === 2;
...
function computeVerticalRTL() {
    ...
    if (htmlStyle) {
        var prop = htmlStyle.getPropertyValue("direction");
    ...
    if (win.document.body) {
        ...
        if (bodyStyle) {
            var prop = bodyStyle.getPropertyValue("direction");
            if (prop && prop.indexOf("rtl") >= 0) {
                rtl = true;
            }
        ...
        var singleChild = void 0;
        var childEl = win.document.body.firstElementChild;
        while (childEl) {
            if (singleChild) {
        ...
        if (singleChild) {
            var singleChildStyle = win.getComputedStyle(singleChild);
            if (singleChildStyle) {
                var prop = singleChildStyle.getPropertyValue("direction");
                if (prop && prop.indexOf("rtl") >= 0) {
                    rtl = true;
                }
